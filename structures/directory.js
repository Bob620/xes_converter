...
class Directory {
	constructor(uri, options={}) {
		...
		this.data.emit(constants.events.directory.NEWDIR, this);
	...
	syncUpdate() {
		if (this.data.files.size !== 0 || this.data.directories.size !== 0 || this.data.totalSubDirectories !== 0) {
			this.data.emit(constants.events.directory.WILLCLEAR, copyDir(this));
			this.data.files = new Map();
			this.data.directories = new Map();
			this.data.totalSubDirectories = 0;
			this.data.emit(constants.events.directory.CLEARED, this);
		}

		this.data.emit(constants.events.directory.WILLUPDATE, {dir: this, sync: true});
		...
		for (const file of files) {
			if (file.name) { // Is node > 11
				if (file.isDirectory()) {
					...
					this.data.emit(constants.events.directory.NEWFILE, {dir: this, fileName: file.name, sync: true});
				}
			} else { // Is node < 11
				...
				if (stats.isDirectory()) {
					...
					this.data.emit(constants.events.directory.NEWFILE, {dir: this, fileName: stats.name, sync: true});
				}
			}
		}

		this.data.emit(constants.events.directory.UPDATED, {dir: this, sync: true});
	}
	...
	async update() {
		...
		this.data.emit(constants.events.directory.WILLUPDATE, {dir: this, sync: false});
		let subDirs = [];
		...
		for (const file of files) {
			if (file.name) {
				if (file.isDirectory()) {
					...
					this.data.files.set(file.name, file);
					this.data.emit(constants.events.directory.NEWFILE, {dir: this, fileName: file.name, sync: false});
				}
			} else { // Is node < 11
				// Get stats for the item
				...
				if (stats.isDirectory()) {
					...
					this.data.files.set(stats.name, stats);
					this.data.emit(constants.events.directory.NEWFILE, {dir: this, fileName: stats.name, sync: false});
				}
			}
		}

		return Promise.all(subDirs).then(() => {
			this.data.emit(constants.events.directory.UPDATED, {dir: this, sync: false});
